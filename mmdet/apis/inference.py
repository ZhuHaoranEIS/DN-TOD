...
def init_detector(config, checkpoint=None, device='cuda:0', cfg_options=None):
    ...
    train_cfg = None
    if config_model.get('type') == 'FCOS':
        train_cfg=config_model.get('train_cfg')
    model = build_detector(config.model, train_cfg=train_cfg, test_cfg=config.get('test_cfg'))
    # config.model.train_cfg = config.get('train_cfg')
